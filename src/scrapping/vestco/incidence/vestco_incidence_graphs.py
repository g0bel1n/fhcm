...
for file in os.listdir():
    if file.endswith(".csv"):
        ...
        df = pd.read_csv(file)
        dfs[key] = df.merge(sheet_df, left_on="brand", right_on="N°Obs")[
            ["incidence", "N°Obs", "Segment de marché", "Niveau de prix"]
        ].set_index("N°Obs")
        dfs[key].to_csv(f"complete_incidence_vestco_{key}.csv")
#%%
